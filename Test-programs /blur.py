...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
	...
	thresh1, binaryImg= cv2.threshold(gray,B,W,cv2.THRESH_OTSU)
	cv2.imshow("resized",binaryImg)
	
	
	blur=cv2.medianBlur(gray,5)
	thresh2, binaryImg2= cv2.threshold(blur,B,W,cv2.THRESH_OTSU)
	cv2.imshow("resizedblur",binaryImg2)
	...
	#resize = cv2.resize(threshB,dim,interpolation=cv2.INTER_AREA)
